  ...
  switch (msg.type) {
    ...
    case "rate":
      if (player && player.setPlaybackRate) {
        suppress(800)
        try {
          player.setPlaybackRate((msg as { playbackRate: number }).playbackRate)
        } catch {}
      }
      break
  }
  ...
  if (!cached) {
    fetchTitle(videoId).then((t) => {
      if (currentVideoId === videoId) {
        videoLabel.textContent =
          t + (queueIndex >= 0 ? `  •  ${queueIndex + 1}/${queue.length}` : "")
...
function renderQueue(): void {
  ...
  queue.forEach((id, i) => {
    ...
    li.innerHTML = `<button class="drag-handle" type="button" title="Drag to reorder" aria-label="Drag to reorder">⠿</button><img class="qthumb" src="${thumb(
      id,
...
;($("#clearQ") as HTMLButtonElement).addEventListener("click", () =>
